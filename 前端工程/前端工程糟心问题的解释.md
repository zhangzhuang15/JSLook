...
### 使用 babel-loader, cannot find module ''
检查 babelrc 是否配置正确，如下情况就会导致这个问题：
```json
{
    "extends": "",
    "presets": [
...
### 用react18了，还会报错： Cannot find variable React 
babelrc里边加入"@babel/preset-react"，并且设置参数"runtime": "automatic"
...
// 如果采取默认配置 "runtime": "classic"，
// <App /> 会被翻译为 React.createElement, 
// 凭空多出来一个 React，自然找不到了
```
   
### copy-webpack-plugin 恶心的拷贝设置
已有 /a/b/c/rr.txt 
```js 
module.exports = {
    plugins: [
        new CopyPlugin({
            patterns: [
                ...
                { from: '/a/b/c/*', to: '/a/b/d'},
                // 拷贝结果： /a/b/d/rr
                { from: '/a/b/c/*', to: '/a/b/d/[name]'},
                // 拷贝结果： /a/b/d/rr.txt 
                // Perfect!
                { from: '/a/b/c/*', to: '/a/b/d/[name][ext]'}
            ]
        })
